...
class ProjectEditForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['users'].queryset = User.objects.exclude(pk=self.instance.created_by.pk)
...
class ProjectCreateForm(forms.ModelForm):
    ...
    def clean_project_name(self):
        ...
        if Projects.objects.filter(project_name=project_name).exists():
            self.add_error('project_name', 'Проект с таким названием уже существует. Измените наименование проекта')
        return project_name
